import Link from "next/link";
import { FaInstagram, FaTwitter, FaYoutube } from "react-icons/fa";

export default function Footer() {
    return (
        <div className=" w-full pt-[48px] px-0 bg-[#f4f7ff]">
            <div className="max-w-[1170px] w-full mx-auto h-full">
                <div>
                    <img className="w-[106px] h-[35px] mx-auto my-12" src="data:image/svg+xml;base64," alt="logo" />
                </div>
                <div className="max-h-full mx-auto flex items-start justify-between border-b border-[#84a0e7] border-opacity-50">
                    <div>
                        <h2 className="text-[18px] text-[#000000CC] font-semibold">Про нас</h2>
                        <ul className="my-4 flex flex-col space-y-3 text-[#1b5bf7] font-semibold">
                            <Link href=''>
                                <li>Услуги</li>
                            </Link>
                            <Link href=''>
                                <li>Команда</li>
                            </Link>
                            <Link href=''>
                                <li>Инструменты</li>
                            </Link>
                            <Link href=''>
                                <li>Клиенты</li>
                            </Link>
                        </ul>
                    </div>
                    <div>
                        <h2 className="text-[18px] text-[#000000CC] font-semibold">Услуги</h2>
                        <ul className="my-4 flex flex-col space-y-3 text-[#1b5bf7] font-semibold">
                            <Link href=''>
                                <li>Разработка мобильных приложений</li>
                            </Link>
                            <Link href=''>
                                <li>Разработка и внедрение ERP систем</li>
                            </Link>
                            <Link href=''>
                                <li>UI / UX дизайн</li>
                            </Link>
                            <Link href=''>
                                <li>IT консалтинг</li>
                            </Link>
                            <Link href=''>
                                <li>Оптимизация инфраструктуры</li>
                            </Link>
                        </ul>
                    </div>
                    <div>
                        <h2 className="text-[18px] text-[#000000CC] font-semibold">Портфолио</h2>
                        <ul className="my-4 flex flex-col space-y-3 text-[#1b5bf7] font-semibold">
                            <Link href=''>
                                <li>Delever</li>
                            </Link>
                            <Link href=''>
                                <li>Sms.uz</li>
                            </Link>
                            <Link href=''>
                                <li>Goodzone</li>
                            </Link>
                            <Link href=''>
                                <li>Iman</li>
                            </Link>
                        </ul>
                    </div>
                </div>
                <div className="w-full h-[50px] flex py-12 items-center px-4 justify-between">
                    <span className="text-[#1b5bf7]">© 2024 Udevs. All rights reserved</span>
                    <div className="flex items-center justify-between gap-4">
                        <Link href=''>
                            <div className="w-9 h-9 rounded-full bg-[#1b5bf71a] text-[#1b5bf7] p-2">
                                <FaInstagram className="size-5" />
                            </div>
                        </Link>
                        <Link href=''>
                            <div className="w-9 h-9 rounded-full bg-[#1b5bf71a] text-[#1b5bf7] p-2">
                                <FaTwitter className="size-5" />
                            </div>
                        </Link>
                        <Link href=''>
                            <div className="w-9 h-9 rounded-full bg-[#1b5bf71a] text-[#1b5bf7] p-2">
                                <FaYoutube className="size-5" />
                            </div>
                        </Link>
                    </div>
                </div>
            </div>
        </div>
    );
}